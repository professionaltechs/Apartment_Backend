...
export const getAllApartment = async (req, res) => {
  try {
    const apartment = await Apartment.find({ isDeleted: { $ne: 1 } }).select({
      __v: 0,
    });
...
export const deleteApartment = async (req, res) => {
  try {
    const apartment = await Apartment.findOne({
      _id: req.body.apartmentId,
    }).select({ price: 0, description: 0, images: 0, __v: 0 });
    if (!apartment) {
      return res.status(200).json({
        message: "no record found",
      });
...
export const permenantlyDeleteApartment = async (req, res) => {
  try {
    const apartments = await Apartment.find({ isDeleted: 1 }).select({
      images: 1,
      _id: 0,
    });
